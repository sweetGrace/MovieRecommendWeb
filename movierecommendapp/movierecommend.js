            ...
            function randomFrom(lowerValue,upperValue)
{
    return Math.floor(Math.random() * (upperValue - lowerValue + 1) + lowerValue);
            ...
            res.render('index',{userid:rows[0].userid,username:rows[0].username,movieinfo:movielist});
    });
});
            ...
            function randomFrom(lowerValue,upperValue)
{
    return Math.floor(Math.random() * (upperValue - lowerValue + 1) + lowerValue);
            ...
            res.render('menu',{userid:rows[0].userid,username:rows[0].username,movieinfo:movielist});
    });
});
            ...
            function randomFrom(lowerValue,upperValue)
{
	...
	//console.log('result point 1');
        ...
        //const testid=Math.floor(Math.random()*100)%17+2
        const username=req.query.username;
        //console.log('recommendation userid is:'+userid);      
        const path = '/input_spark';
        //调用Spark程序为用户推荐电影并把推荐结果写入数据库
        let spark_submit = spawnSync('/usr/local/spark/bin/spark-submit',['--class', 'recommend.MovieLensALS',' ~/IdeaProjects/Film_Recommend/out/artifacts/Film_Recommend_jar/Film_Recommend.jar', path, 1],{ shell:true, encoding: 'utf8' });
